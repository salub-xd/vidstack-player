  ...
  return (
    <VolumeSlider.Root className="volume-slider group relative mx-[7.5px] inline-flex h-10 w-full max-w-[80px] cursor-pointer touch-none select-none items-center outline-none aria-hidden:hidden">
      <VolumeSlider.Track className="relative ring-media-focus z-0 h-[5px] w-full rounded-sm bg-white/30 group-data-[focus]:ring-[3px]">
        <VolumeSlider.TrackFill className="bg-blue-300 absolute h-full w-[var(--slider-fill)] rounded-sm will-change-[width]" />
      </VolumeSlider.Track>
...
export function Time({ thumbnails }: {thumbnails?: string}) {
  return (
    <TimeSlider.Root
      noSwipeGesture
      className="time-slider group relative mx-[7.5px] inline-flex h-8 w-full cursor-pointer touch-none select-none items-center outline-none"
    >
        ...
        {(cues, forwardRef) =>
          cues.map((cue) => (
            <div
              ...
              ref={forwardRef}
            >
              <TimeSlider.Track className="relative ring-media-focus z-0 h-[3px] w-full rounded-sm bg-white/30 group-data-[focus]:ring-[3px]">
                <TimeSlider.TrackFill className="bg-red-500 absolute h-full w-[var(--chapter-fill)] rounded-sm will-change-[width]" />
